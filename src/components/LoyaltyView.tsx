          ...
          {isEditing ? (
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button 
                  ...
                  className="flex-1 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold py-2 rounded-lg transition-colors"
                >
                  Batal
                </button>
              </div>
            ...
            <>
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">{player.name}</h3>
                <button 
              ...
              <div className="flex items-center gap-2">
                <span className={`text-xs font-bold px-2 py-0.5 rounded-md border ${
                  player.category === 'SD' ? 'bg-blue-100 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-500/20' :
                  player.category === 'SMP' ? 'bg-purple-100 dark:bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-200 dark:border-purple-500/20' :
                  'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 border-slate-200 dark:border-slate-600'
                }`}>
                  {player.category}
                </span>
                <span className="text-xs text-slate-500 dark:text-slate-400">
                  Total: {player.totalLifetimeHours} jam
                </span>
              </div>
            </>
          )}
        ...
        <>
          <div className="mb-6">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-slate-600 dark:text-slate-400 font-medium">Progress Minggu Ini</span>
            ...
            {isPromoEligible && (
              <p className="text-xs text-slate-500 mt-2">
                {5 - player.accumulatedHours > 0 
                  ? `Main ${5 - player.accumulatedHours} jam lagi untuk dapat voucher!` 
                  : 'Target tercapai! Voucher didapatkan.'}
              </p>
            )}
          </div>
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Catatan</span>
              <button 
                onClick={() => setIsEditing(true)}
            ...
            <div 
              onClick={() => setIsEditing(true)}
              className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3 text-sm text-slate-600 dark:text-slate-300 min-h-[60px] cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-900/80 transition-colors border border-transparent hover:border-indigo-500/20 group"
            >
              {player.notes ? (
                player.notes
              ) : (
                <span className="text-slate-400 italic group-hover:text-indigo-400 transition-colors">Klik untuk menambahkan catatan...</span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-700/50">
            <div className="flex items-center gap-2">
              ...
              <div>
                <div className="text-xs text-slate-500 font-medium uppercase">Voucher</div>
                <div className="text-lg font-bold text-slate-900 dark:text-white">{player.vouchers}</div>
              </div>
            </div>
            <button 
              onClick={() => onUseVoucher(player.id)}
              disabled={player.vouchers === 0}
              className="px-4 py-2 bg-slate-900 dark:bg-white text-white dark:text-slate-900 rounded-xl text-sm font-bold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-indigo-600 dark:hover:bg-indigo-500 dark:hover:text-white transition-all shadow-lg shadow-slate-900/10 active:scale-95"
            >
              Gunakan
            </button>
          </div>
